...
int get_load_from_server(int s_port, int sno)
{
    ...
    if (connect(sockfd, (const struct sockaddr *)&s_addr, sizeof(s_addr)))
    {
        perror("Could not connect for load request\n");
        exit(1);
    }
        ...
        do
        {
            ...
            else
            {
                ...
                if (fork() == 0)
                {
                    ...
                    if (connect(s_sockfd, (const struct sockaddr *)&s_addr, sizeof(s_addr)) < 0)
                    {
                        perror("Could not connect for servicing\n");
                        exit(1);
                    }
